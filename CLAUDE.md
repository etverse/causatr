...
- **Categorical (multinomial) outcome is a vector-valued estimand, not multivariate outcomes** (Phase 23a-1). A single K-level factor outcome (`model_fn = nnet::multinom`) makes the estimand the K-vector `P(Y = k | do(A = a))` per intervention (the class probabilities sum to 1) — the outcome-side mirror of multinomial *treatment*; it is NOT several outcome columns. Detection is post-fit (`is_multinom_outcome()` → `inherits(model, "multinom")`, `R/family.R`); `compute_contrast()` branches **early** to `compute_contrast_multinom()` (`R/contrast.R`) and returns before the scalar tail, so the scalar `causatr_result` is **byte-identical** (no `class` column, matrix `vcov`, `class_labels = NULL`). The per-class assembly adds a `class` column to `estimates`/`contrasts`, stores `vcov` as a per-class list, and records `class_labels`; the scalar delta-method block was extracted to `compute_pairwise_contrasts()` and is shared so the contrast math cannot diverge (OR is one-vs-rest per class). Variance is **bootstrap only** (`variance_bootstrap_multinom()`, `R/variance_bootstrap.R` — flattens the `k_int × K` surface class-major, reuses `process_boot_results()`, slices per-class blocks; `refit_gcomp()` replays `nnet::multinom` unchanged); the analytic sandwich is Phase 23a-2. **Point gcomp only**; `check_categorical_outcome()` (`R/checks.R`, run in `causat()` before `prepare_data()`) rejects SNM by design (`causatr_snm_categorical_outcome`) and IPW/AIPW/matching/longitudinal/transport (`causatr_categorical_outcome_unsupported`), and `contrast()` rejects sandwich (`causatr_categorical_outcome_sandwich`) and stochastic interventions. Orthogonal dimensions (treatment type, deterministic intervention, estimand, contrast scale, IPCW, weights, `by`) ride the per-class loop for free. Oracle: large-n softmax truth + **exact** `marginaleffects::avg_predictions()` parity (~1e-15) on causatr's own multinom fit. Ordinal (`MASS::polr`, cumulative `P(Y ≥ k)`) is Phase 23b and reuses this schema.
