...
- For IPW and matching with saturated MSMs (Y ~ A), the predict-then-average approach in `compute_contrast()` gives the same result as reading off model coefficients (the Jacobian is trivial). This is correct but slightly redundant — kept for architectural consistency across methods. **Caveat:** the saturated MSM means IPW and matching also silently ignore any `A:modifier` interaction term in `confounders`, because `confounders` is used only for the propensity / match model, not the MSM. This is the core Phase 8 issue (`PHASE_8_INTERACTIONS.md`). Point gcomp does NOT have this problem because its outcome model uses the full `confounders` RHS directly.
...
- **Effect modification across methods is the Phase 8 topic** (`PHASE_8_INTERACTIONS.md`). Current status: point gcomp handles `~ L + sex + A:sex` correctly. IPW and matching silently ignore the `A:sex` term because their MSM is hardcoded `Y ~ A`. ICE handles current-period `A:sex` but not `lag1_A:sex` (heterogeneity compressed across periods).
