...
test_that("AIPW agrees with gcomp and IPW on well-specified DGP", {
  ...
  fit_gc <- causat(d, outcome = "Y", treatment = "A", confounders = ~L,
                   estimator = "gcomp")
  fit_ipw <- causat(d, outcome = "Y", treatment = "A", confounders = ~L,
                    estimator = "ipw")
  fit_aipw <- causat(d, outcome = "Y", treatment = "A", confounders = ~L,
                     estimator = "aipw")
...
test_that("AIPW bootstrap agrees with sandwich (within 30%)", {
  ...
  res_sand <- contrast(fit, interventions = ivs, reference = "a0",
                       ci_method = "sandwich")
  res_boot <- contrast(fit, interventions = ivs, reference = "a0",
...
test_that("AIPW rejects longitudinal data", {
  expect_error(
    causat(
      data.frame(Y = 1, A = 1, L = 1, id = 1, time = 1),
      outcome = "Y", treatment = "A", confounders = ~L,
      estimator = "aipw", id = "id", time = "time"
    ),
...
test_that("AIPW rejects multivariate treatment", {
  d <- data.frame(Y = rnorm(50), A1 = rbinom(50, 1, 0.5),
                  A2 = rbinom(50, 1, 0.5), L = rnorm(50))
  expect_error(
    causat(d, outcome = "Y", treatment = c("A1", "A2"),
           confounders = ~L, estimator = "aipw"),
    "Multivariate"
  )
})
...
test_that("AIPW recovers sex-stratified ATE with effect modification", {
  ...
  fit <- causat(d, outcome = "Y", treatment = "A", confounders = ~ L + A:sex,
                estimator = "aipw")
  result <- contrast(fit,
    interventions = list(a1 = static(1), a0 = static(0)),
    reference = "a0",
