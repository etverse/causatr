...
# ---------------------------------------------------------------------------
# Non-gaussian outcome variants of the delay DGP (Poisson and Gamma)
# ---------------------------------------------------------------------------

#' Delay DGP with Poisson or Gamma final-step outcome
#'
#' Re-generates `Y` from the standard linear-gaussian delay SCM using a Poisson
#' or Gamma draw on the same linear predictor `b0 + bA*cumA + bL*L_tau + bL0*L0`.
#' The absorbing treatment structure and covariate dynamics are identical to
#' [glmtp_delay_data()]; only the outcome noise distribution changes, so the
#' same forward-MC truth applies (both distributions share the log-link mean).
#'
#' @param family Character. `"poisson"` or `"gamma"`.
#' @param n,seed,tau Passed to [glmtp_delay_data()].
#' @return Same list as [glmtp_delay_data()]: `$data` and `$params`.
glmtp_delay_data_family <- function(
  family = c("poisson", "gamma"),
  n = 4000L,
  seed = 1L,
  tau = NULL
) {
...
#' Forward-MC truth for the delay regime with a Poisson or Gamma outcome
#'
#' Both Poisson and Gamma use a log link, so `E[Y] = exp(lp)` regardless of
#' which distribution is used. This function returns `mean(exp(lp))` over the
#' natural-history simulated population — identical for both families.
#'
#' @param window Integer delay.
#' @param family Character. `"poisson"` or `"gamma"` (output is the same).
#' @param n_mc,seed,params See [glmtp_delay_forward_truth()].
#' @return Numeric scalar, the true \eqn{E[Y^d]}.
glmtp_delay_forward_truth_family <- function(
  window,
  family = c("poisson", "gamma"),
  n_mc = 2e6,
  seed = 1L,
