...
test_that("gcomp transport: target_subset stored on fit", {
  ...
  fit_tgt <- causat(d, "Y", "A", ~L, estimator = "gcomp", target = "S",
                    target_subset = "target")
  fit_all <- causat(d, "Y", "A", ~L, estimator = "gcomp", target = "S",
                    target_subset = "all")
...
test_that("gcomp transport: check_transport_inputs rejects ATT/ATC", {
  ...
  expect_snapshot(
    causat(d, "Y", "A", ~L, estimator = "gcomp", target = "S",
           estimand = "ATT"),
    error = TRUE
  )
  expect_snapshot(
...
test_that("gcomp transport corrects study bias vs. naive study-only estimate", {
  ...
  fit_transport <- causat(d, "Y", "A", ~ L + A:L, estimator = "gcomp",
                          target = "S", target_subset = "target")
  res_transport <- contrast(fit_transport,
    interventions = list(a1 = static(1), a0 = static(0)),
...
test_that("gcomp transport: sandwich SE is plausible (ratio to bootstrap in (0.5, 2))", {
  ...
  fit <- causat(d, "Y", "A", ~ L + A:L, estimator = "gcomp",
                target = "S", target_subset = "target")
  ivs <- list(a1 = static(1), a0 = static(0))
  res_sw <- contrast(fit, interventions = ivs, type = "difference",
                     ci_method = "sandwich")
  res_bt <- contrast(fit, interventions = ivs, type = "difference",
