...
- **Saturated MSM in IPW/matching.** Any finding in `R/variance_if.R`'s `variance_if_ipw()` / `variance_if_matching()` branches or `R/variance_bootstrap.R`'s IPW/matching paths that depends on a non-saturated `Y ~ A` MSM is **latent, not active**, because `fit_ipw()` and `fit_matching()` hardcode `Y ~ A` and reject `A:modifier` terms via `check_confounders_no_treatment()`. Flag as **Phase 4 prerequisite** and defer — do not fix pre-Phase-4 unless the user explicitly asks.
