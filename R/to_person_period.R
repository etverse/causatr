...
to_person_period <- function(
  ...
  # The reshape assumes one row per individual (each (var, t) cell
  # is its own column). A duplicated id in the wide input would
  # produce malformed long data with mangled time alignment, so
  # reject it up front.
