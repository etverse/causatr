...
### 6.5 Longitudinal IPCW: the conservative approximation

For longitudinal treatments with per-period censoring models $\gamma_1,
\ldots, \gamma_K$, the full correction requires cross-derivatives
$A_{\beta_k, \gamma_j}$ for all $k \geq j$, projected through the ICE
cascade's accumulated sensitivity $d_k$. This is technically supported by
stacked M-estimation theory [@stefanski2002mestimation; @tsiatis2006semiparametric, Ch. 10],
but the cascade's $d_k$ vectors are built incrementally during the
backward loop and cannot be evaluated inside a `numDeriv` closure.

causatr currently **omits** the longitudinal censoring Channel 2 correction.
This makes the sandwich SE conservative (larger than the true asymptotic SE).
Empirically the ratio is $\approx 0.94$ (sandwich/bootstrap), confirming the
omission is small.

**Recommendation:** For longitudinal IPCW, use `ci_method = "bootstrap"` for
the tightest CIs. The sandwich remains valid for coverage guarantees.

No existing package (`lmtp`, `ltmle`, `gfoRmula`) implements the longitudinal
censoring Channel 2 correction — all use bootstrap variance in this setting.
