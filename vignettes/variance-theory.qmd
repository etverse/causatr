...
nocite: |
  @hernan_whatif, @vdl2003unified, @zivich2022delicatessen
---
...
3. **Section 3 — Two-channel IF for $\hat\mu$.** A causal estimand
   ...
   small otherwise.**
4. **Section 4 — The core estimators.** G-computation, IPW, and matching
   are three choices of estimating equations plugged into the same
   machinery. Each gets its own IF, but all are aggregated the same
   way. AIPW combines the g-comp and IPW influence functions into a
   doubly-robust IF; its variance follows the same framework.
...
$\sum_i \psi(O_i; \hat\beta) = 0$. The M-estimation framework
[@huber1964robust; @stefanski2002mestimation] unifies them under two conditions:
...
estimating equations, and the causal estimators in Sections 4–5
[@stefanski2002mestimation].
:::
...
property [@huber1967behavior; @white1982maximum].
:::
...
package [@zeileis2004econometric; @zeileis2006object] provides variants for clustered data
...
This is the *HC0* heteroscedasticity-consistent estimator [@white1980heteroskedasticity].
...
the estimating equations and whether Channel 1 is zero. AIPW combines the g-comp
and IPW influence functions into a single doubly-robust IF — its variance
derivation follows the same Channel 1 + Channel 2 structure; see
`R/variance_if_aipw.R` for the implementation.
...
conservative — it **overestimates** the SE [@hirano2003efficient; @lunceford2004stratification].
For the **ATT**, the effect can go either direction.
...
### 4.4 Summary: one method, multiple estimators

All methods use the same variance formula — the IF squared and summed
...
|---|---|---|---|
| **G-comp** | Outcome model + mean eq. | 1 (outcome) | Standard or user-cluster |
| **IPW** | Propensity + MSM + mean eq. | 2 (propensity, MSM) | Standard or user-cluster |
| **Matching** | Outcome model + mean eq. | 1 (outcome) | Cluster-robust on subclass |
| **ICE** | $K\!+\!1$ outcome models + mean eq. | $K\!+\!1$ (backward chain) | Standard or user-cluster |
...
within clusters for matching, or within any user-supplied design cluster via
`contrast(cluster = ...)`; directly otherwise). Matching's subclass cluster is
structural — `contrast(cluster = ...)` is rejected there with a
`causatr_bad_cluster` abort because layering a user cluster on top would
double-count within-subclass rows. For `gcomp`, `ipw`, and `ice`, the same
sum-within-cluster-then-square primitive that Section 4.3 derives for
matching is exposed directly via the `cluster` argument; numerically
equivalent to `sandwich::vcovCL(type = "HC0")` applied to the final
predict-then-average step (up to a small $G/(G-1)$ cluster-count factor).
...
score $\psi_k$, the IF of $\hat\mu$ is [@hampel1986robust; @huber1981robust]:
...
1. **Stacked estimating equations** [@stefanski2002mestimation; @zivich2024ice]:
   concatenate all models' estimating equations and apply the generic
   sandwich to the full stack.
2. **Semiparametric efficiency theory** [@tsiatis2006semiparametric]: derive the efficient
...
counterfactual mean under a treatment strategy in longitudinal data
[@robins1986newapproach; @bang2005doubly; @zivich2024ice]. It fits $K+1$ outcome models
backward in time:
...
## 6. IPCW: variance under outcome censoring

When outcome data are missing at random (MAR) and causatr fits a censoring
model $P(C = 0 \mid A, L)$, the IPCW weights become estimated nuisance
parameters. The variance of the causal estimand must account for this
estimation uncertainty — otherwise the sandwich SE is conservative.

### 6.1 The censoring model as a third nuisance block

The censoring model adds a block to the stacked estimating equations.
For **point-treatment g-computation** with IPCW, the stack becomes:
...
\end{pmatrix}
= \begin{pmatrix}
\text{censoring model score} \\
\text{outcome model score (IPCW-weighted)} \\
\text{target estimand}
\end{pmatrix}
$$

where $\theta = (\gamma, \beta, \mu)^T$, $\gamma$ are censoring model
parameters, $\beta$ are outcome model parameters, and $\mu$ is the marginal
mean.

### 6.2 The block-triangular bread

The bread matrix has the same block-triangular structure as in Section 3.5,
now with one extra block:
...
The key new block is $A_{\beta\gamma}$: the cross-derivative of the outcome
model's score with respect to the censoring parameters. This captures how
changes in IPCW weights (driven by $\gamma$) affect the outcome model fit.
...
In causatr, this is computed via `numDeriv::jacobian()` on a closure that
perturbs the IPCW weights while holding $\hat\beta$ fixed.

### 6.3 The Channel 2 correction for censoring

Back-substitution through the bread yields an additional Channel 2 correction
term in the IF:

$$
\text{IF}_i = \underbrace{\text{Ch1}_i}_{\text{covariate sampling}}
+ \underbrace{\text{Ch2}_i^{(\text{outcome})}}_{\text{outcome model}}
- \underbrace{\text{Ch2}_i^{(\text{censoring})}}_{\text{censoring model}}
$$

The censoring correction is:

$$
\text{Ch2}_i^{(\text{censoring})}
= g_\gamma^T \, A_{\gamma\gamma}^{-1} \, \psi_{\gamma,i}
$$

where $g_\gamma = A_{\beta\gamma}^T \, h_\beta$ and $h_\beta =
A_{\beta\beta}^{-1} J_\mu^T$ projects the marginal-mean gradient through
the outcome model bread.

The minus sign on the censoring correction is deliberate:
@robins1994aipw showed this correction **reduces** variance
by accounting for the information in the censoring model. Ignoring it
(treating IPCW weights as known) is conservative.

### 6.4 Point-treatment IPW with IPCW

For IPW + IPCW, the stack has four blocks: censoring model, propensity model,
MSM, and target. The bread becomes $4 \times 4$ block-triangular:
...
Two off-diagonal blocks feed into the MSM score: $A_{\beta\gamma}$ (IPCW
weights) and $A_{\beta\alpha}$ (propensity weights). Each generates an
independent Channel 2 correction.

::: {.callout-note}
## Implementation note (point IPW + IPCW)

The block structure above assumes the propensity is estimated *without* the
censoring weight, so $\gamma$ reaches the MSM only through $A_{\beta\gamma}$. The
current point IPW implementation instead fits the propensity IPCW-weighted on the
uncensored rows, which adds a $\gamma \to \alpha$ coupling the analytic sandwich
does not yet carry — the point IPW + IPCW SE is therefore off by ~3–7% per arm
(the point estimate is consistent). The standardized estimator (unweighted
propensity, matching the longitudinal path of §6.5) is a scoped follow-up. For
correct point IPW + IPCW standard errors today, use `ci_method = "bootstrap"`.
:::

### 6.5 Longitudinal IPCW: the per-period censoring block

For longitudinal treatments the censoring model is fit per period, with
coefficients $\gamma_1, \ldots, \gamma_K$. Whether — and how — that block
enters the sandwich depends on the estimator.

**Longitudinal IPW.** The final-period Hájek MSM is weighted by the cumulative
IPCW weight, so the censoring block contributes a single cross-derivative
$A_{\beta\gamma}$ exactly as in the point IPW case (Section 6.4), with the
per-period censoring scores stacked. causatr propagates it
(`compute_ipw_ipcw_correction_longitudinal()`): the MSM-score sensitivity is
formed against the stacked $\gamma$ via `numDeriv::jacobian()`, then projected
through each period's censoring-model influence function. This term is
**load-bearing** — when censoring depends on treatment it can move the
treated-arm SE by several percent (recovering the @robins1994aipw efficiency
gain). Crucially, the per-period propensity models are estimated *unweighted*
(the IPCW weight reweights the MSM only — the standard separate-then-multiply
construction, @hernan_whatif Ch. 12.6 & 17), so $\gamma$ reaches $\mu$ only
through the MSM (a $\gamma \to \beta$ path); there is no $\gamma \to \alpha$
term. Validated to $\sim 10^{-4}$ against a `delicatessen` M-estimation stack.

**Longitudinal AIPW.** The stacked-EE sandwich (Section 5) carries the
per-period $\gamma$ block in $\theta = (\alpha, \beta, \gamma, \mu)$, threading
the IPCW weight into each outcome score so the numerical bread captures
$\gamma \to \beta \to \mu$ mechanically. By the double-robust orthogonality of
the augmented estimator this cross-term is **near-zero** (it moves the marginal
mean SE by $\sim 0.03\%$): the block is wired and carried for an exactly correct
sandwich, not to fix a sizeable bias.

**Longitudinal ICE (g-computation).** The pooled-ICE sandwich does *not* yet
carry the censoring Channel 2 correction: the full term requires
cross-derivatives $A_{\beta_k, \gamma_j}$ projected through the cascade's
incrementally-built sensitivity $d_k$, which is not available inside a single
`numDeriv` closure. The ICE sandwich is therefore conservative under IPCW
(larger than the true asymptotic SE). **Recommendation:** for longitudinal ICE
with IPCW use `ci_method = "bootstrap"` for the tightest CIs; the sandwich
remains valid for coverage guarantees.

No existing package (`lmtp`, `ltmle`, `gfoRmula`) implements an analytic
longitudinal censoring Channel 2 correction — all use bootstrap variance in
this setting.
...
7. **User-supplied cluster** (`contrast(cluster = ...)`): the same
   sum-within-cluster-then-square aggregation matching uses internally is
   exposed directly on gcomp, IPW, and ICE. Numerically equivalent to
   `sandwich::vcovCL(type = "HC0")` on the final predict-then-average step;
   rejected on matching because subclass clustering is already structural.

8. **Chained estimators** (ICE): $K\!+\!1$ model corrections computed by
   backward recursion of sensitivities.

9. **IPCW** adds a censoring-model block to the stacked equations. The
   censoring Channel 2 correction **reduces** variance by accounting for
   weight estimation uncertainty. For longitudinal IPCW, the correction is
   omitted (conservative); use bootstrap for tighter CIs.

10. **The delta method** ($\color{#31A354}{J} V_\beta
...
## 8. Numeric variance tiers (custom `model_fn`)

The analytic IF engine in `variance_if()` requires access to the model's
score residuals and bread matrix. Standard GLMs (`stats::glm`) and GAMs
(`mgcv::gam`) expose these via `family$mu.eta` / `family$variance` and
`$Vp`, respectively. But users can pass any fitting function via
`model_fn =` (e.g. `MASS::glm.nb`, `betareg::betareg`), and those
models may not expose the standard GLM internals.

causatr handles this with a two-tier numeric fallback (`variance_if_numeric()`):

### Tier 1: `sandwich::estfun` available

If `sandwich::estfun(model)` returns the per-observation score matrix
$\psi_i$ and `sandwich::bread(model)` returns the bread, the fallback
recovers the **full IF** (Channel 1 + Channel 2 + cross-term) exactly,
matching the analytic main path. The marginal-mean Jacobian $J$ is
computed via `numDeriv::jacobian()` on `predict()` instead of
analytically.

This applies to `MASS::glm.nb` (negative binomial) and most model
classes with a `sandwich` method.

### Tier 2: plain delta shortcut

If no `estfun` method exists (e.g. `betareg::betareg`), the fallback
computes $V_1 + V_2$ where $V_1 = (1/n^2) \sum_i \text{Ch.1}_i^2$ is
the Channel 1 variance and $V_2 = J \, V_\beta \, J^T$ is the delta-method
approximation for Channel 2. This **drops the cross-term**
$(1/n^2)(\text{Ch.1}^T \text{Ch.2} + \text{Ch.2}^T \text{Ch.1})$ and emits
a `causatr_tier2_fallback` warning so the user knows the SE is slightly
miscalibrated.

For Tier 2 models, `ci_method = "bootstrap"` gives exact coverage.

### Which tier does my `model_fn` use?

| `model_fn` | Tier | Exact? |
|---|---|---|
| `stats::glm` | Analytic (main path) | Yes |
| `mgcv::gam` | Analytic (GAM path) | Yes |
| `MASS::glm.nb` | Tier 1 (numeric + estfun) | Yes |
| `betareg::betareg` | Tier 2 (delta shortcut) | Approximate (use bootstrap for exact) |

If unsure, run with `ci_method = "sandwich"` first — if you see a
`causatr_tier2_fallback` warning, the model is on Tier 2.

## Bootstrap confidence-interval flavour

When `ci_method = "bootstrap"`, `contrast()` resamples the whole pipeline and
stores the per-intervention replicate matrix. Two standard intervals can be
read off those replicates [@efron1993bootstrap, ch. 13; @davison1997bootstrap, sec. 5.2],
selected with `boot_ci`:

- **`"percentile"` (default)** — the empirical $\alpha/2$ and $1-\alpha/2$
  quantiles of the replicates. It is transformation-respecting and bounded by
  the estimand's support, so a relative risk or odds ratio gets a strictly
  positive interval and a probability stays in $[0, 1]$. Contrast intervals use
  the quantiles of the contrast evaluated on each replicate's per-intervention
  means, so difference / ratio / odds-ratio are all handled by the one recipe.
- **`"normal"`** — the Wald interval from the bootstrap standard error,
  $\hat\theta \pm z_{1-\alpha/2}\,\widehat{sd}$. Symmetric; best when the
  bootstrap distribution is roughly Gaussian.

Both come from the same replicates, so the choice costs nothing extra and
leaves the point estimate, SE, and vcov untouched — only the interval bounds
move. Every surface (`print()`, `plot()`, the `estimates` / `contrasts`
tables, `tidy()`) honours the recorded choice; `confint(res, boot_ci = ...)`
overrides it on demand.
...
For the SNM blip parameters and the pooled population-averaged blip, `boot_ci`
applies the same way; the `by`-stratified averaged blip keeps its delta-method
(normal) interval because it has no per-stratum replicates.
