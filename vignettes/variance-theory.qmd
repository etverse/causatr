...
### 3.6 Pseudo-code: the full IF for a marginal mean
...
gcomp_influence <- function(model, data_a, target_idx) {
  ...
  IF_correction <- as.numeric(X_fit %*% h) * residuals  # n_fit x 1

  # Combine: full IF = Channel 1 + n * Channel 2
  # Factor of n from A^{-1} = n * (X'WX)^{-1}
  IF <- (n / n_t) * IF_direct
...
$\color{#31A354}{J_0}\color{#3182BD}{A_0^{-1}}\psi_{0,i}$ from Section 3.2,
...
chained_influence <- function(models, data_iv, target) {
  ...
  IF <- rep(0, n)
  IF[target] <- (n / n_target) * (pseudo_final[target] - mu_hat)  # Channel 1
  ...
  for (step in 0:K) {
    ...
    # Step 1: Gradient g_k = d mu_hat / d beta_k
    if (step == 0) {
    ...
    # Channel 2 for model k: n_k * d_{k,i} * r_{k,i}
    r_vec <- rep(0, n)
    r_vec[fit_ids_k] <- r_k
    IF <- IF + n_k * d * r_vec
  }
