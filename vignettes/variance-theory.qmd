...
tt(out, caption = "Sample mean IF variance matches the textbook SE") |>
  ...
  format_tt(j = 2, digits = 6, num_fmt = "significant")
```
...
tt(out, caption = "Two-channel IF for g-comp: manual vs `causatr::variance_if`") |>
  format_tt(j = "quantity", markdown = TRUE) |>
  format_tt(j = 2:4, digits = 6, num_fmt = "significant")
```
...
variance (Channel 1²) happens to be small relative to the parameter
...
Channel-2 form given in @eq-if-ipw. causatr routes this through
`WeightIt::glm_weightit()` + `sandwich::estfun(asympt = TRUE)`, which
returns per-individual IFs already adjusted for propensity estimation.
