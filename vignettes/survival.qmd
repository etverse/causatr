...
implementation (Phase 6 scaffolding) exposes a fit-only path via
...
informative error. Competing-risks analysis is also reserved for Phase 6.
...
- `competing`: reserved for competing-risks analysis. Supplying a non-`NULL`
  ...
  proper sub-distribution / Aalen–Johansen path is implemented in Phase 6.
...
pointing to this open scope. Track progress in `PHASE_6_SURVIVAL.md`.

```{r}
#| eval: false
# Will abort until Phase 6 contrast support lands:
result <- contrast(
  fit_surv,
...
For a quick sanity check before the Phase 6 contrast path is wired up, you
...
- `PHASE_6_SURVIVAL.md` — design notes and the outstanding work list.
