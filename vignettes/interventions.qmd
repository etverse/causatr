...
We use the NHEFS dataset throughout, dropping rows with missing outcome or
education (the only covariate with NA values). `qsmk` (binary) is the
treatment for the static, dynamic, and IPSI examples. `smokeintensity`
(continuous) is the treatment for shift, scale, and threshold examples.
