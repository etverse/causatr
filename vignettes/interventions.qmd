...
`grace_period()` and `carry_forward()` are **natural-history** modified
treatment policies (G-LMTPs): the intervened treatment at time $t$ depends on
the *counterfactual natural-value history* of treatment, so they cannot be
written as `dynamic()` rules (which see only the observed lag). They are
estimated by an augmented-data sequential regression and require a longitudinal
g-computation fit with a discrete treatment — see the
[longitudinal vignette](longitudinal.html) for runnable examples.
