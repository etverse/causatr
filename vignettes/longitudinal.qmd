...
**What's not yet supported (chunk 10a).** Effect-modification
(`A:modifier` in `confounders`), `stabilize = "marginal"` /
`numerator =` for stabilized weights, multivariate longitudinal
treatment, and per-period `ipsi()` are rejected with `_pending`
classed errors pointing at the deferred follow-up chunks. Use
`estimator = "gcomp"` (ICE) for those scenarios today.
...
Y <- 10 + (2 + 1.5 * sex) * (A0 + A1) + L0 + L1 + rnorm(n)

d_em <- rbind(
  data.frame(id = 1:n, time = 0L, A = A0, L = NA_real_,
