...
**What's not yet supported (chunk 10c).** Effect-modification
(`A:modifier` in `confounders`), multivariate longitudinal treatment,
and per-period `ipsi()` are rejected with `_pending` classed errors
pointing at the deferred follow-up chunks. Use `estimator = "gcomp"`
(ICE) for those scenarios today.
