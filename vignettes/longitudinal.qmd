...
**natural-value history of treatment** --- the treatment a patient *would* take
absent the intervention, at $t$ *and* at earlier periods. The canonical example
is a **grace period / delay**: postpone treatment initiation by one period
relative to when the patient would naturally start. These are
**G-LMTPs** (Díaz, Williams, Morzywołek & Rudolph 2026).

A grace period is **not** a `dynamic()` rule. The standard ICE recursion
conditions on the *observed* lagged treatment (`lag1_A`), but under a delay the
natural value at $t$ differs from the observed value once the policy has
perturbed the trajectory (treatment-state feedback). Writing
`dynamic(\(d, trt) d$lag1_A)` runs without error but silently targets the wrong
estimand. causatr instead uses the paper's **augmented-data sequential
regression**, carrying every possible natural-treatment-history sequence as a
label through the backward recursion.

Use `grace_period(window)` (delay initiation by `window` periods) or
`carry_forward()` (hold everyone at their baseline level). They require a
longitudinal g-computation fit with a **discrete** treatment, and are inferred
with the ID-cluster bootstrap.

**DGM.** A 4-period binary **absorbing** treatment (once started, stays on) with
covariate feedback, so the natural history under a delay genuinely differs from
...
d_glmtp <- data.frame(
  ...
  time = rep(seq_len(tau), n),
  L0 = rep(L0, each = tau),
  A = as.vector(t(A)),
  L = as.vector(t(L)),
  Y = NA_real_
)
d_glmtp$Y[d_glmtp$time == tau] <- Y
...
```{r}
#| label: fig-grace
#| fig-cap: "Counterfactual means under a one-period delay, carry-forward, and the natural course, with ID-cluster bootstrap 95% CIs."
#| fig-width: 6
#| fig-height: 2.6
library(tinytable)
library(tinyplot)

tt(res_grace$estimates)

est <- res_grace$estimates
with(est, tinyplot(
...
Delaying initiation by one period lowers the counterfactual mean relative to the
natural course (fewer treated person-periods); carry-forward (hold at baseline)
lowers it further. The augmented engine reproduces the forward-simulation truth
for these policies, whereas a naive `dynamic(lag1_A)` rule is off by tens of
percent --- see `tests/testthat/test-glmtp.R`, which replicates the Díaz et al.
(2026) Section-6 delay result.
