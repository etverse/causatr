...
The AIPW engine is cross-validated against **`delicatessen`** (Zivich et al.
2024), a Python M-estimation library that implements the same stacked
estimating-equation sandwich for AIPW (Robins, Rotnitzky & Zhao 1994).
Because Python is not a test-suite dependency, the cross-check uses
pre-computed reference values generated by `data-raw/aipw_reference.py` on
the same DGP (same seed, same data written to CSV):

- Binary treatment, linear-Gaussian DGP, `n = 5000`, `seed = 42`:
  μ₁, μ₀, ATE, and all three sandwich SEs match delicatessen to 4 decimal
  places.
- Continuous treatment, shift δ = 1, linear-Gaussian DGP, `n = 5000`,
  `seed = 99`: μ_shift, μ_natural, and the contrast SE match delicatessen
  to 4 decimal places (pushforward density-ratio formula
  W_i = exp(δ(A_i − μ(L_i) − δ/2) / σ²)).

The reference values are pinned in `test-aipw.R` with `tolerance = 0.005`.
See `data-raw/aipw_reference.py` for the full stacked-EE Python implementation.
...
- **`survey::svyglm()`** (combined with a user-supplied weights column) is
  ...
  survey-weight handling in its tests — including the direct
  `causat(weights = svydesign(...))` convenience path that auto-extracts
  the design's sampling weights and first-stage PSU into the
  contrast-time cluster — a vignette comparison would mostly duplicate
  that coverage.
- **`sandwich::vcovCL()`** is the oracle for causatr's general
  cluster-robust sandwich (`causat(cluster = ...)` /
  `contrast(cluster = ...)`). Agreement on a saturated `Y ~ A + L`
  model is pinned to machine precision after the `G/(G-1)` small-cluster
  correction in `test-cluster-sandwich.R`.
