...
compare_row <- function(label, result, name, ref_name,
  ...
  data.table::data.table(
    quantity = label,
    source = c("causatr", ref_name),
    estimate = c(row$estimate, ref_est),
    se = c(row$se, ref_se),
    ci_lower = c(row$ci_lower, ref_est - z * ref_se),
    ci_upper = c(row$ci_upper, ref_est + z * ref_se)
  )
}
```
...
estimate + SE under a normal-approximation CI. The output is a two-row
`data.table` you can print verbatim.
