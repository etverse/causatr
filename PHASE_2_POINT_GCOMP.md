...
2. `contrast()` → `compute_contrast()` in `R/contrast.R`
...
3. Sandwich variance in `R/variance_sandwich.R`
   - G-comp: `sandwich::sandwich(model)` for V_β (Huber–White HC0)
   - Propagated via `compute_vcov_marginal()`: J V_β Jᵀ using `numDeriv::jacobian()`

4. Bootstrap variance in `R/variance_bootstrap.R`
   - Full-pipeline resampling via `boot::boot()`
   - `refit_gcomp()`: resample → filter censored/NA → refit model_fn
...
- **Sandwich via J V_β Jᵀ** (marginaleffects approach) rather than stacked estimating equations (Zivich et al.). Asymptotically equivalent; simpler to implement. Stacked EE via `geex` planned for Phase 5 (ICE).
- **NA handling**: rows with missing confounders produce NA predictions → excluded from target population automatically
...
|---|---|---|
| NHEFS g-formula ATE | ≈ 3.5 kg (CI: 2.6–4.5) | Passing |
| Simulated DGP ATE = 3 | ≈ 3 | Passing |
| Binary outcome RD ≈ 0.33 | ≈ 0.33 | Passing |
| Risk ratio (binary outcome) | > 1 | Passing |
| Shift intervention (dY/dA = 2) | ≈ −2 | Passing |
| Scale, threshold, dynamic | Correct direction | Passing |
| Sandwich SE ≈ bootstrap SE | Within factor of 2 | Passing |
| GAM via model_fn = mgcv::gam | ≈ 3 | Passing |
| Censoring handling | ≈ 3 (random censoring) | Passing |
| ATT estimand | Valid, non-NA | Passing |
| Subset estimand | ≈ 3 (constant effect DGP) | Passing |
