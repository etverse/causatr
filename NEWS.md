<!--
  Claude / reviewer note: keep this file up to date on every PR. A
  new user-visible change — a new feature, a bug fix, a behavior
  change, a renamed / removed API, an upgraded test cell in
  FEATURE_COVERAGE_MATRIX.md, or a new vignette — gets a bullet
  under the current development-version section, grouped by date.
  Refactors, formatting passes, and internal renames that don't
  change behavior do NOT belong here. When cutting a release, move
  "development version" to the new version number and start a fresh
  "development version" block. Prefer one bullet per theme over one
  bullet per commit; readers care about what changed, not how many
  commits it took.
-->
