...
| delicatessen cross-check — point estimates (tol 1e-4) | ✅ | test-snm-triangulation.R |
| delicatessen cross-check — sandwich SEs (tol 0.3%) | ✅ | test-snm-triangulation.R |
...
**delicatessen fixture:** `tests/testthat/fixtures/snm_longitudinal_delicatessen.csv` (generated by `data-raw/snm_longitudinal_reference.py` from shared fixture `data-raw/snm_longitudinal_fixture.csv`).

**Remaining chunks (all ❌):** 18g (gesttools cross-check), 18h (categorical/count), 18i (bootstrap), 18j (S3 dispatch), 18k (documentation + vignette).
